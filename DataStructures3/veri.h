...
};	//Ilk derinlik arama fonksiyonu bir agac koku ve aranacak deger parametrelerini alir
bool BFS(Dugum* p, int a)
{
    queue<Dugum*> queue;
    int counter = 0;
    queue.push(p);
...
};//Ilk genislik arama fonksiyonu bir agac koku ve aranacak deger parametrelerini alir


void IkiliSiralamaAgaci::agacKur(int* p) {



    int deðer = *p;

    Dugum* tempParent = new Dugum;
    Dugum* tenpSearch = kok;
    while (tenpSearch != NULL)
    {
        tempParent = tenpSearch;
        if (deðer == (int)tenpSearch->veri)
        {
            return;
        }
        else if (deðer < (int)tenpSearch->veri)
        {
            tenpSearch = tempParent->sol;
        }
        else
        {
            tenpSearch = tempParent->sag;
        }
    ...
    else if (deðer < (int)tempParent->veri)
    {
